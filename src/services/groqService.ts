...
export function buildRefinePrompts(text: string, options: RefineOptions): RefinePrompts {
  ...
  if (processingMode === "Remove Em Dashes Only") {
    ...
    prompt = `You are an expert human editor. Your task is to "de-AI" or "humanize" the following text, which was likely generated by an LLM. Rewrite it so it sounds like a person wrote it — not a language model.
...
1. CUT FILLER PHRASES
   ...
   - Remove all adverbs: really, just, literally, genuinely, honestly, simply, actually, deeply, truly, fundamentally, inherently, inevitably, interestingly, importantly, crucially
   - Replace business jargon: "navigate" -> handle, "unpack" -> explain, "lean into" -> accept, "landscape" -> situation, "game-changer" -> significant, "double down" -> commit, "deep dive" -> analysis, "circle back" -> revisit
   ...
   - Remove vague declaratives: "The implications are significant", "The reasons are structural", "The stakes are high", "The consequences are real"

2. BREAK FORMULAIC STRUCTURES
   - Remove binary contrasts: "Not because X, but because Y" — state Y directly instead
...
3. USE ACTIVE VOICE WITH REAL SUBJECTS
...
4. BE SPECIFIC
   ...
   - No lazy extremes (every, always, never, everyone) doing vague work
   - Say the specific thing, not the abstraction about it

5. PUT THE READER IN THE ROOM
   - No narrator-from-a-distance voice ("Nobody designed this", "This happens because", "People tend to")
   - "You" beats "People". Specifics beat abstractions

6. VARY RHYTHM
   - Mix short and long sentences — three consecutive sentences of similar length sounds robotic
   - Two items in a list beats three
   ...
   - No staccato fragmentation — don't stack short punchy sentences
...
9. Convert all American English spelling and phrasing to British English (e.g., "color" to "colour", "optimize" to "optimise").
